...
class KoreaInvestment:
        ...
        if exchange == "KRX":
            if any(ticker.startswith(code) for code in futures_codes):
                ...
                if side == "buy":
                    ...
                    body |= KoreaFuturesOrderBody(**body, SLL_BUY_DVSN_CD="02", SHTN_PDNO=ticker, ORD_QTY=amount, ORD_DVSN_CD=f"{KoreaFuturesOrderType.market}")
                else:
                    body |= KoreaFuturesOrderBody(**body, SLL_BUY_DVSN_CD="01", SHTN_PDNO=ticker, ORD_QTY=amount, ORD_DVSN_CD=f"{KoreaFuturesOrderType.market}")
            else:
    ...
    #     body = self.base_order_body.dict()
    #     query = KoreaFuturesInquireBalanceQuery(**body).dict()
